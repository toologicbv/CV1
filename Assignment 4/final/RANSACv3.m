function [tparams, best_matches]=RANSACv3(img_path1, img_path2, N, f1, f2, T, plot_result)
...
if plot_result && (size(best_matches,2) > 0.8 * size(T, 2))
    ...
    function [I2_t]=matlab_affine_trans(im, M)
        ...
        % tried both MATLAB versions
        % tform = affine2d(M');
        % I2_t = imwarp(im, tform);
        % swap translation to lower row in position 1 and 2
        % which is required for MATLAB function
        M(3, 1:2) = M(1:2,3)';
        M(1:2,3) = 0;
